...
from Qiyang_TEXT.Layers import Bi_LSTM
...
def get_default_params():
    return tf.contrib.training.HParams(
        ...
        num_lstm_nodes=[32, 32],  # 第一层LSTM的模型表示维度与第二层LSTM模型表示维度
...
lstm_1 = Bi_LSTM(hps.num_embedding_size, hps.num_lstm_nodes[0], 'Bilayer_1')  # 第一层LSTM，参数为1、词向量维度 2、LSTM表示维度 3、LSTM的名称
lstm_2 = Bi_LSTM(hps.num_lstm_nodes[0], hps.num_lstm_nodes[1], 'Bilayer_2')  # 第二层LSTM
...
def create_model(hps, vocab_size, num_classes):
    ...
    # 将embedding_inputs投入到lstm模块当中去进行计算
    outputs_1 = lstm_1.calculate(embeding_inputs)
    # 将第一层lstm所处理的结果投入到第二层lstm再进行训练
    last = lstm_2.calculate(outputs_1)

    # 我们将lstm输出的结果的时间步给浓缩掉，我们要将[batch_size,numsteps,dimension]=>[batch_size,dimension]
    last_r = tf.reshape(last, [hps.batch_size, hps.num_lstm_nodes[1], hps.num_timesteps])
    last_rr = tf.reshape(last_r, [-1, hps.num_timesteps])
    with tf.variable_scope('pick_out'):
        W_pick = tf.get_variable('weights', [hps.num_timesteps, 1])
        B_pick = tf.get_variable('bias', [1, hps.num_lstm_nodes[1]])
    outputs_p1 = tf.matmul(last_rr, W_pick)
    outputs_p2 = tf.reshape(outputs_p1, [hps.batch_size, hps.num_lstm_nodes[1]])
    outputs_p3 = tf.nn.tanh(outputs_p2 + B_pick)
    ...
    with tf.variable_scope('fc', initializer=fc_init):  # initializer 此范围内变量的默认初始值
        ...
        fc1 = tf.layers.dense(outputs_p3,
                              hps.num_fc_nodes,
                              activation=tf.nn.relu,
                              name='fc1')
        # 为避免过拟合，我们将一些全连接层当中的单元给停用掉。不参与到后续的计算，当然，每个epoch我们会随机去选择一些单元停用。
        fc1_dropout = tf.nn.dropout(fc1, 0.8)  # 0.8表示我们保留可用的单元的比例

        # 对于输出层，我们只有一个单元，logits表示我们的输出结果[batch_size,1]
        logits = tf.layers.dense(fc1_dropout, 1, name='fc2')
        logits = tf.reshape(logits, (logits.shape[0],))
        outputs = tf.cast(outputs, tf.float32)

    with tf.name_scope('metrics'):
        # loss = 1/batch_size*Σlabelslog(logits)+(1-labels)log(1-logits)我们使用内置的函数即可完成计算
