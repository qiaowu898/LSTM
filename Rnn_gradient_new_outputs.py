...
def create_model(hps, vocab_size, num_classes):
    ...
    with tf.variable_scope('rnn_nn', initializer = lstm_init):
        ...
        Rnn_outputs = tf.Variable(tf.zeros([batch_size,hps.num_lstm_nodes[0],1])
                                  ,trainable=False)
        for i in range(num_timesteps): # 按照 词语的数量 进行
            ...
            # RNN内部的运算
            h = tf.tanh(
                tf.matmul(embeding_input, wx) + tf.matmul(h, wh) + b
            )
            Rnn_outputs = tf.concat([Rnn_outputs,tf.reshape(h,[hps.batch_size,hps.num_lstm_nodes[0],1])],axis=2)
    ...
    Rnn_outputs_r = tf.reshape(Rnn_outputs,[-1,hps.num_timesteps])
    weighted_outputs = tf.matmul(Rnn_outputs_r,output_weights)
...
with tf.Session() as sess:
    ...
    np.savez('RNN_newoutputs_results.npz',gradient=gradient_max_save,
